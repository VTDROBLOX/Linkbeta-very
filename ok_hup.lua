...
local HopToggle = Tabs.Hop:AddToggle({
    ...
    Callback = function(Value)
        ...
        if IsHopping then
            ...
            task.spawn(function()
                local count = 0
                -- Vòng lặp bắt ép spam đúng 100 lần liên tục hoặc dừng khi bạn tắt nút gạt
                while IsHopping and count < 100 do
                    count = count + 1
                    print("Đang thử kết nối tìm phòng lần thứ: " .. count)
                    ...
                    if request_fn then
                        ...
                        local visited = _G.VisitedServersForHop or {}  
                        _G.VisitedServersForHop = visited

                        local function tp(job)
                            job = tostring(job)
                            local ok = pcall(function()
                                TeleportRemote:InvokeServer("teleport", job)
                            end)
                            if ok then return true end

                            local ok2 = pcall(function()
                                TeleportService:TeleportToPlaceInstance(game.PlaceId, job, game.Players.LocalPlayer)
                            end)
                            return ok2
                        end

                        -- Gọi request lấy dữ liệu phòng từ Render của bạn
                        ...
                        if ok and res then
                            local body = pcall(function() return HttpService:JSONDecode(res.Body) end) and HttpService:JSONDecode(res.Body)
                            if body then
                                ...
                                if server_list and type(server_list) == "table" then
                                    ...
                                    if #valid_jobs > 0 then
                                        local selected = valid_jobs[math.random(1, #valid_jobs)]
                                        print("Đã khóa mục tiêu phòng Trăng Tròn:", selected)
                                        visited[selected] = true
                                        
                                        local success = tp(selected)
                                        if success then
                                            task.wait(2)
                                        end
                                    else
                        ...
                        warn("Executor không hỗ trợ hàm Request!")
                        IsHopping = false
                        if HopToggle then HopToggle:Set(false) end
                        break
                    end
